...
with tab4:
    ...
    # Clean the dataset again to ensure consistency
    df_svm = pd.read_csv("heart.csv")
    df_svm.drop_duplicates(inplace=True)
    ...
    outlier_mask = pd.Series([False] * df_svm.shape[0], index=df_svm.index)
    for col in continuous_cols:
        outlier_mask |= is_outlier_iqr(df_svm[col])
    df_svm = df_svm[~outlier_mask]
    ...
    st.subheader("🎯 Decision Boundary of SVM (RBF Kernel) in PCA Space")
    ...
    ax.set_title("SVM Decision Boundary (PCA Projection)")
    ax.set_xlabel("PCA Component 1")
    ax.set_ylabel("PCA Component 2")
    ax.grid(True)
    st.pyplot(fig)

    st.subheader("🎯 SVM Decision Boundary with PCA-reduced Heart Data")

    # Train linear SVM on PCA data
    ...
    sns.scatterplot(x=X_pca[:, 0], y=X_pca[:, 1], hue=y, palette="Set1", edgecolor="k", ax=ax)
    ax.set_title("SVM Decision Boundary with PCA-reduced Heart Data")
    ax.set_xlabel("PCA Component 1")
    ax.set_ylabel("PCA Component 2")
    ax.grid(True)
    ...
    # (Assuming y_test and y_pred_svm already defined earlier)
